...
\ [hp:
\ [prima sache, das ganze; nur das ANS-ige gefasel nervt:
\ [dieses 'Requiring' ist quatsch, denn 'required' wird hier nur dann was,
\ [wenn selbiges ohnehin vorhanden ist. wozu waere die umwandlung 'v>f'
\ [rsp. 'f>v' bezuegl der ans-4th-darstellung sonst noch von nutzen?!
\ [auszer M+ ist strenggenommen nichts von dem hier genannten wirklich 
\ ['required', wenn auch insbes. D+, D-, DNEGATE die sache sehr erleichtern.
\
\ Requiring the Floating-Point wordset (namely D>F F* F- F/ F>D FDUP FLITERAL
\                                              FLOOR FOVER and FSWAP).
\ [hp:
\ [what might a floating point numbers implementation be good for, 
\ [the prereqisite of which is a complete, other one?! anyway, the 
\ [above words are 'required' for conversion to/from the ans-4th 
\ [ieee binary f.p. formats, only...
\
...
\ [hp:
\ [niemand 'required' 
\ [	'0<>' oder '<>' oder 'false' oder gar 'marker'!
\ [ebensowenig
\ [	'?do' oder '2>r' oder '2r>' oder 'nip' oder 'to' oder 'true' oder 'tuck' oder 'value'...
\ [(geistvolle trickserei mit 2r> und >r >r ist sicher nicht 'required'.)
\ [auch 'PICK' - nur fuer m** - ist leicht ersetzbar.
\ [d.h. allesamt unsinn, rsp. ganz sicher nicht 'required'.
\
\ Requiring DU< from the Double-Number Extensions word set.
\
\ Requiring CS-ROLL from the Programming-Tools Extensions word set.
\ [hp
\ [auch nicht sonderlich geistreich, und bei weniger wirrem aufbau der betr.
\ [worte unnoetig:
\
...
\ [hp
\ [ { 2 pick } entbehrlich z.b. mit:
\ [ : um** ( u1 u2--ud) swap dup do 1. ROT 0 ?DO j mu* LOOP leave loop ;
...
: reduce ( dv--v)  setvars  dv>udv? >R
                   BEGIN  2DUP d0<>
                   WHILE  2OVER 2DUP d0<> IF  +d/mod  THEN	( dz dn -- dz dn dz | dz dr dq )
                          2DUP num>? IF  next! DROP  DONE	( dz dr dq -- dz dr dq uq f --dun: dz dr ql <== ??? )
\ - should read ----------------------------> 2drop <----------------------- [hp] --
                          ROT ROT den>? IF  next!  DONE		( dz dr uq -- dz uq ur f    --dun: dz )
                          next! 2SWAP				( dz 
                   REPEAT THEN THEN 2DROP 2DROP
                   prev.numerator @  R> ?negate
                   prev.denominator @ ;
\ -- [hp] --------------------------------------------------------------------------
\ begin 2dup or while 2over 2dup or if +d/mod endif	( dz dn -- dz dn dz | dz dr dq )
\ 2dup numerator @ mu* prev.numerator @ 0 d+ ud>+n?	( dz dr dq -- dz dr dq uq f    )
\ if denominator @ prev.denominator ! denominator ! numerator @ prev.numerator ! numerator ! drop DONE ( dz dr dq uq -- dz dr ql (1)
\ rot rot denominator @ mu* prev.denominator @ 0 d+ ud>+n? ( dz dr dq uq -- dz dr uq ur f )
\ if denominator @ prev.denominator ! denominator ! numerator @ prev.numerator ! numerator ! DONE ( dz dr uq ur -- dz dr (2)
\ denominator @ prev.denominator ! denominator ! numerator @ prev.numerator ! numerator ! 2swap ( dz dr dq uq -- dr dz )
\ repeat endif(2) endif(1) 2drop 2drop
\ ----------------------------------------------------------------------------------

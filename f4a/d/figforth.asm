...
;---------------------------------------------------------------------
; labels adapted to 386+ and "fasm" mnemonics
;	CMOVE	=>	CMOVE4
;	LEAVE		LEAV
;	AT		AT4
;	DP		DP4
...
;	LODB		LODSB
;	plain stupid:	JZ	RET
; => nasm does the assembly, but, expectedly, does no working binary!
;---------------------------------------------------------------------
;
;		( Page 2 )
;
...
FDD		EQU	0		;Density(0=single,1=double)
;
;
;		( Page 3 )
;
...
PNUM1: 		DW	ONEP
		DW	DUP,	TOR
		DW	CAT,	BASE,	AT4,	DIGIT
		DW	ZBRAN,	PNUM2-$-2
		DW	SWAP,	BASE,	AT4,	USTAR
		DW	DROP,	ROT,	BASE,	AT4
		DW	USTAR,	DPLUS
		DW	DPL,	AT4,	ONEP
		DW	ZBRAN,	PNUM3-$-2
		DW	ONE,	DPL,	PSTOR
...
INTE4: 		DW	QSTAC
		DW	BRAN,	INTE5-$-2
INTE2: 		DW	HERE,	NUMB,	DPL,	AT4,	ONEP
		DW	ZBRAN,	INTE6-$-2
		DW	DLITE
		DW	BRAN,	INTE7-$-2
INTE6: 		DW	DROP,	LITER
...
;
	...
	ALIGN
		DW	TSCALC - 10
BLKRD:		DW	$ + 2
		MOV	byte[DSKERR+2],0	;reset error flag
...
RSLW4:		DW	ZERO,	PREV,	AT4,	STORE	;This  buffer
							; is no good!
		DW	QERR
RSLW5:		DW	SEMIS
;
